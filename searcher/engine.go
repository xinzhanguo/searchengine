...
func (e *Engine) getShard(id uint64) int {
	return int(id % uint64(e.Shard))
}

func (e *Engine) getShardByWord(word string) int {
	return int(utils.StringToInt(word) % uint64(e.Shard))
}
...
func (e *Engine) addInvertedIndex(word string, id uint64) {
	...
	if !arrays.ArrayUint64Exists(ids, id) {
		ids = append(ids, id)
	}
...
// 移除删去的词
func (e *Engine) optimizeIndex(id uint64, newWords []string) ([]string, bool) {
	// 判断id是否存在
	e.Lock()
	defer e.Unlock()
...
func (e *Engine) removeIdInWordIndex(id uint64, word string) {
...
func (e *Engine) getDifference(id uint64, newWords []string) ([]string, []string, bool) {
	shard := e.getShard(id)
...
// GetDocById 通过id获取文档
func (e *Engine) GetDocById(id uint64) []byte {
	shard := e.getShard(id)
	key := utils.Uint64ToBytes(id)
...
func (e *Engine) RemoveIndex(id uint64) error {
	//移除
	e.Lock()
	defer e.Unlock()
